// interfaces/IRelationalService.ts
import type {
  CreateResult,
  UpdateResult,
  RelationalDoc,
...
export interface IRelationalService {
  ...
  queryByRefsWithData<T = any>(
    collection: string,
    refs: Record<string, string>,
    dataFilters: Record<string, any>
  ): Promise<(RelationalDoc<T> & { id: string })[]>;

  queryByRefFlattened<T = any>(
    collection: string,
    refKey: string,
    refValue: string
  ): Promise<(T & { id: string })[]>;
